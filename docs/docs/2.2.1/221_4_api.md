...
* **Preemptive Running**: Manual operations can now **preempt** the existing watering queue, allowing them to run immediately. This applies to [**Manual Station Run**](#11-manual-station-run-cm), [**Run-Once Program**](#17-run-once-program-cr), and [**Manual Program Start**](#12-manually-start-a-program-mp).
    * A new parameter `pre` is available for these commands to insert new zones at the front of the queue instead of adding them to the end.
...
|`flwrt`| Flow count window (seconds). Default is `30` (firmware-defined).|
...
| `uwt`             | Weather adjustment method. `0`: Manual; `1`: Zimmerman; `2`: Auto Rain Delay; `3`: ETo; `4`: Monthly. <u>NOTE</u>: Previously `uwt.bit[7]` was used to flag California water restriction. That bit is no longer used; instead, water restriction is indicated in the `wto` (weather options) variable.|
...
**<span class="hl">Usage</span>**: <code>http://os-ip/cm?**pw**=xxx&**sid**=xx&**en**=x&**t**=xxx&**ssta**=x&**pre**=x</code>
...
| `pre`    | <span class="hl">Preemptive mode (applies when opening a zone)</span>. `0`: append to queue (default); `1`: preempt, i.e. insert ahead of existing queued zones.|
...
* `os-ip/cm?pw=xxx&sid=3&en=1&t=300&pre=1`: open station 4 for 5 minutes, preempt others.
...
**<span class="hl">Usage</span>**: <code>http://os-ip/mp?**pw**=xxx&**pid**=xx&**uwt**=x&**pre**=x</code>

|Parameter|Meaning |
|:--------|:-------|
| `pid`   | Program index (starting from `0`).|
| `uwt`   | Use weather (`1`: apply current watering level; `0`: run at 100%).|
| `pre`   | <span class="hl">Queue behavior.</span> `0`: append the new program to the end of the queue; `1`: insert at front (preemptive); `2`: **replace**, i.e. reset all zones, clear queue, then start the new program. If `pre` is omitted, the default behavior is `2` (backward compatible).|
...
* `os-ip/mp?pw=xxx&pid=0&uwt=0`: reset all zones, then start program 1 at 100% watering level. As `pre` is omitted, this behavior is backward compatible with previous firmwares.
* `os-ip/mp?pw=xxx&pid=1&uwt=1&pre=0`: <span class="hl">append program 2 at the end of the queue, using the current watering level.</span>
* `os-ip/mp?pw=xxx&pid=1&uwt=1&pre=1`: <span class="hl">insert program 2 at the front of the queue.</span>
* `os-ip/mp?pw=xxx&pid=1&uwt=1&pre=2`: <span class="hl">reset all zones, then start program 2 using the current watering level.</span>
...
**<span class="hl">Usage</span>**: <code>http://os-ip/cr?**pw**=xxx&**t**=[x,x,...,x]&**cnt**=xxx&**int**=xxx&**uwt**=xxx&**anno**=xxx&**pre**=x</code>
...
| `pre`    | <span class="hl">Queue behavior</span> (same as `/mp`). `0`: append to end; `1`: insert at front; `2`: **replace** (reset all zones, clear queue, then start). Default is `2` if `pre` is omitted. |
...
* `os-ip/cr?pw=xxx&t=[60,0,60,0,60,0,60,0]`: reset all zones, then start a run-once program that turns on stations 1, 3, 5,7 for `1` minute each. As `pre` is omitted, this behavior is compatible with previous firmwares. 
* `os-ip/cr?pw=xxx&t=[60,0,60,0,60,0,60,0]&uwt=1&pre=0`: append the run-once program at the end of the queue, using the current watering level.
* `os-ip/cr?pw=xxx&t=[60,0,60,0,60,0,60,0]&uwt=0&pre=1`: insert the run-once program at the front of the queue, using 100% watering level.
